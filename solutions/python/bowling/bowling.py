...
class GameOverError(Exception):
    """Raised when a roll is attempted after the game is over."""
    pass


class GameNotOverError(Exception):
    """Raised when a score is requested before the game is over."""
    pass
...
class BowlingGame:
    ...
    def roll(self, pins: int) -> None:
        ...
        if self.is_game_over:
            raise GameOverError("Cannot roll after game is over.")
        if pins < 0:
    ...
    def score(self) -> int:
        ...
        Raises:
            GameNotOverError: If the game is not yet over.
        """
        if not self.is_game_over:
            raise GameNotOverError("Cannot score until the game is over.")
        roll_index = 0
        total_score = 0
        for _ in range(MAX_FRAMES):
            frame_score = self.rolls[roll_index] + self.rolls[roll_index + 1]
            total_score += frame_score
            if self.rolls[roll_index] == MAX_PINS:
                total_score += self.rolls[roll_index + 2]
                roll_index += 1
            else:
                roll_index += 2
                if frame_score == MAX_PINS:
